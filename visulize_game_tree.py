...
def build_game_tree(current_number, player1_score, player2_score, depth, parent_node_id):
    ...
    if current_number <= 10:
        return graphviz.Digraph(node_attr={'shape': 'box'}), node_id, f"Winner: {'Player 1' if player1_score > player2_score else 'Player 2' if player2_score > player1_score else 'Draw'}"
    ...
    if depth % 2 == 0:  # Player 1's turn
        if current_number % 2 == 0:
          child_dot1, child_node_id1, winner1 = build_game_tree(current_number // 2, player1_score, player2_score + 2, depth + 1, node_id)
          dot.subgraph(child_dot1)
        if current_number % 3 == 0:
          child_dot2, child_node_id2, winner2 = build_game_tree(current_number // 3, player1_score + 3, player2_score, depth + 1, node_id)
          dot.subgraph(child_dot2)
    else:  # Player 2's turn
        if current_number % 2 == 0:
            child_dot1, child_node_id1, winner1 = build_game_tree(current_number // 2, player1_score + 2, player2_score, depth + 1, node_id)
            dot.subgraph(child_dot1)
        if current_number % 3 == 0:
            child_dot2, child_node_id2, winner2 = build_game_tree(current_number // 3, player1_score, player2_score + 3, depth + 1, node_id)
            dot.subgraph(child_dot2)
